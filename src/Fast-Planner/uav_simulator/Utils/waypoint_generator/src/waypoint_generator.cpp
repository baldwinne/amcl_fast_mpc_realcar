...
void publish_waypoints_vis() {
    ...
    poseArray.header.frame_id = std::string("odom"); //改成了odom 源码是world
...
/*    if (!is_odom_ready) {
    ...
    if (waypoint_type == string("circle")) {
        ...
        load_waypoints(n, trigged_time);
            ...
            // 创建一个新的 PoseStamped 对象用于存储转换后的目标点
            
             static tf::TransformListener listener;
             geometry_msgs::PoseStamped transformed_pt;
            try {
                    // 等待 transform
                    listener.waitForTransform("odom", "map", ros::Time(0), ros::Duration(1.0));
                    // 进行坐标系转换
                    listener.transformPose("odom", *msg, transformed_pt);
                     // 清空并更新 waypoints
                    waypoints.poses.clear();
                    waypoints.poses.push_back(transformed_pt);
                    // 发布更新后的 waypoints
                    publish_waypoints_vis();
                    publish_waypoints();
